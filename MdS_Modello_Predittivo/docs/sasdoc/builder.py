...
class Builder:
    ...
    def __init__(self, dir_path):
        self._srcs = [SrcPath(p) for p in sorted(search_dir_for_sas(dir_path))]
    ...
    def write_reST_files(self, dst_dir):
        for src in self._srcs:
            ...
            print("sas2rst", src.sas_path())
            if "test" in str(src.sas_path()):
                print("  SKIPPING")
                continue
            ...
            if not isinstance(macro_lib, MacroLib):
                print(f"  SKIPPING - {macro_lib}")
                continue
