...
_SIGNATURE_RE = re.compile(r"\s*(?P<name>\w+)\s*(\((?P<params>.*)\))*", re.MULTILINE | re.DOTALL)
...
def simple_sas_2_rst(sas_src):
    ...
    if macro_match is None:
        ...
        name, position_params, keyword_params = parse_signature(macro_match.group("signature"))
        ...
        if shrt_cmmt_match is None and long_cmmt_match is None:
            comment = None
        elif shrt_cmmt_match is not None and (
            long_cmmt_match is None or shrt_cmmt_match.start() < long_cmmt_match.start()
        ):
            comment = shrt_cmmt_match.group(1)
        else:
            comment = long_cmmt_match.group(1)

        macro = SASMacro(name, comment, position_params=position_params, keyword_params=keyword_params)
