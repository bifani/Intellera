proc printto log="C:\Users\r.blaco\Desktop\scripts Modello predittivo 20220706\logs\L1_PS_AMB.txt" new;
run;
...
%macro L1_PS_AMB(ID_CLASSIFICAZIONE=,reg=);
...
quit;
	...
	%IF %eval(&check_indicatore. > 0) %THEN %DO;
		...
		proc sql;
			...
			proc sql;
			create table temp.L1_PS_AMB_WRK_01_&reg as
				select
						...
						AMB.ID_ANONIMO,
						1 as NM_QUANTITA,
						COALESCE(AMB.data_dimiss,MDY(7,1,ANNO)) AS DT_EROGAZIONE,
						(AMB.importo_lordo) as AM_VALORE_LORDO,
						...
						"" as cod_branca /*non c'� il codice branca*/
				from output.PS_&reg AMB,
					temp.prestazioni_&reg CRIT
				WHERE
					ID_CLASSIFICAZIONE = &ID_CLASSIFICAZIONE.

					AND(
						(CRIT.FL_INI='0' AND compress(AMB.ID_PREST_PRIM, ".")=CRIT.ID_PRESTAZIONE) OR
						(CRIT.FL_INI='1' AND substr(compress(AMB.ID_PREST_PRIM,"."),1,length(CRIT.ID_PRESTAZIONE))=CRIT.ID_PRESTAZIONE)
						)
			;quit;

		PROC SQL;
			CREATE TABLE TEMP.L1_PS_AMB_WRK_&reg AS
				SELECT
					ID_CLASSIFICAZIONE,
					ID_INDICATORE,
					ID_ANONIMO,
				...
				FROM (
					SELECT
						...
						FROM
							...
							temp.L1_PS_AMB_WRK_01_&reg AMB_F
						where
		...
		proc delete data = temp.L1_PS_AMB_WRK_01_&reg temp.prestazioni_&reg;
		run;
...
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=010);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=020);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=030);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=041);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=042);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=050);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=060);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=070);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=080);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=090);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=100);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=110);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=120);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=130);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=140);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=150);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=160);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=170);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=180);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=190);
%L1_PS_AMB(ID_CLASSIFICAZIONE=9,reg=200);
