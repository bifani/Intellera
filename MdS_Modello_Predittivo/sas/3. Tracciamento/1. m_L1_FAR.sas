...
proc printto log="C:\Users\r.blaco\Desktop\scripts Modello predittivo 20220706\logs\L1_FAR_PROVA.txt" new;
run;
...
%macro L1_FAR(ID_CLASSIFICAZIONE=, reg=); /* LIVELLO 1 - FARMACEUTICA versione  */
	...
	proc sql;
		create table temp.far_&reg._filt as select *
			...
			where id_atc <> "" /*in (select id_atc from temp.atc_list)*/; /*-> corretto*/
	quit;
