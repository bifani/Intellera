...
class DataFactory(ABC):
    ...
    def eda_autoviz(self, verbose: int = 1, max_rows_analyzed: int = 100000, max_cols_analyzed: int = 100):
        """
        EDA with Autoviz.

        .. seealso:: https://github.com/AutoViML/AutoViz
        """
        X = self.get_data("df")
    ...
    def eda_dabl(self, target_col: str, verbose: int = 10):
        """
        EDA with Data Analysis Baseline Library.
        ...
        try:
            ...
            plot(X, target_col=target_col, verbose=verbose)
        except Exception as e:
            raise ValueError(f"{e}")

    def eda_dataprep(self, c0: Union[str, int] = None, c1: Union[str, int] = None):
        """
        EDA with DataPrep.

        .. seealso:: https://github.com/sfu-db/dataprep
        .. seealso:: https://towardsdatascience.com/dataprep-eda-accelerate-your-eda-eb845a4088bc
        """
        X = self.get_data("df")
        self.logger.info("")
        self.logger.info("Dataprep Plot")
        try:
            ...
            raise ValueError(f"{e}")
        self.logger.info("")
        self.logger.info("Dataprep Correlation")
        try:
            from dataprep.eda import plot_correlation

            fig = plot_correlation(X, c0, c1)
            fig.show()
        except Exception as e:
            raise ValueError(f"{e}")
        self.logger.info("")
        self.logger.info("Dataprep Missing")
        try:
            from dataprep.eda import plot_missing

            fig = plot_missing(X, c0, c1)
            fig.show()
        except Exception as e:
            raise ValueError(f"{e}")
    ...
    def eda_dtale(self):
        ...
        .. seealso:: https://github.com/man-group/dtale
        .. seealso:: https://towardsdatascience.com/introduction-to-d-tale-5eddd81abe3f
        """
        X = self.get_data("df")
        import dtale
    ...
    def eda_klib(self):
        ...
        .. seealso:: https://github.com/akanz1/klib
        """
        X = self.get_data("df")
        X_num = X[self.get_columns("numeric")]
        X_obj = X[self.get_columns("object")]
        import klib

        if len(X_obj.columns) != 0:
            klib.cat_plot(X)
    ...
    def eda_lux(self, c0: list = []):
        """
        EDA with Lux.

        .. seealso:: https://github.com/lux-org/lux
        .. seealso:: https://towardsdatascience.com/quick-recommendation-based-data-exploration-with-lux-f4d0ccb68133
        """
        X = self.get_data("df")
        if len(c0) != 0:
            X.intent = c0
    ...
    def eda_missingno(self, n: int = None):
        """
        EDA with Missingo.

        .. seealso:: https://github.com/ResidentMario/missingno
        """
        ...
        msno.matrix(X, figsize=(8, 6), fontsize=10)
        # msno.bar(X, figsize=(8, 6),fontsize=10)
        # plt.tight_layout()
        msno.heatmap(X, figsize=(8, 6), fontsize=10)
        plt.tight_layout()
        # msno.dendrogram(X, figsize=(8, 6),fontsize=10)
        # plt.tight_layout()

    def eda_pandas_profiling(self, n: int = None, explorative: bool = False, minimal: bool = True):
        """
        EDA with Pandas Profiling.

        .. seealso:: https://github.com/pandas-profiling/pandas-profiling
        """
        X = self.get_data("df")
    ...
    def eda_sweetviz(self, condition_series=None, names: list = []):
        """
        EDA with Sweetviz.

        .. seealso:: https://github.com/fbdesignpro/sweetviz
        .. seealso:: https://towardsdatascience.com/powerful-eda-exploratory-data-analysis-in-just-two-lines-of-code-using-sweetviz-6c943d32f34
        """
        X = self.get_data("df")
        ...
        if condition_series is not None:
            SVCI = sv.compare_intra(X, condition_series=condition_series, names=names)
            SVCI.show_notebook()

    def eda_ydata(self, get_warnings=None):
        """
        EDA with ydata.

        .. seealso:: https://github.com/ydataai/ydata-quality
        .. seealso:: https://towardsdatascience.com/a-data-scientists-guide-to-identify-and-resolve-data-quality-issues-1fae1fc09c8d
        """
        X = self.get_data("df")
        ...
        if get_warnings is not None:
            dq.get_warnings(test=get_warnings)

    def eda_vaex(self, c0: Union[str, int] = None, c1: Union[str, int] = None):
        """
        EDA with Vaex.

        .. seealso:: https://github.com/vaexio/vaex
        """
        X = self.get_data("df")

        import vaex

        X.vaex.plot1d(X[c0], limits="99.7%", show=True)
        X.df.plot.scatter(c0, c1)
